...
> Quiet Reveal / 安静显影：以暖白留白承载真实内容，让个人状态与真实收藏像一份仍在生长的档案，在滚动中逐层出现。
...
| Visual density | 4 / 10 | 保留呼吸感，但让真实档案与媒体内容更早进入视野 |

### 首页 V2：个人活档案索引

- **页面任务**：访客先认识“旺角西多士”这个具体的人，再进入已有真实内容的游戏与音乐收藏；首页不承担空内容预告。
- **结构顺序**：人物首屏 → 一段个人自述 → 游戏与音乐媒体墙 → 最近记录 → 直接联系。
- **首屏证据**：公开头像、22 岁、海南文昌、正在学习成为 AI 训练师、4 款游戏、6 首音乐与可计算的 Steam 公开时长。
- **视觉焦点**：人物首屏只使用一张真实头像；收藏段是全页唯一深色反转，也是媒体密度最高的段落。
- **删减规则**：没有真实内容的经历、书籍与电影不出现在主导航、首页、收藏馆入口、页脚或 404 推荐中；原路由与数据模型保留。
- **排版规则**：Hero 标题最大 6rem、移动端最多三行；普通章节标题明显小于 Hero，正文控制在舒适阅读宽度，不再用多个同级超大标题拉长页面。
- **布局规则**：桌面端采用 7/5 人物档案分栏与 7/5 收藏错位，移动端严格单列；靠内容、细线与比例分组，不新增通用卡片容器。
- **动效规则**：首屏头像使用一次裁切显影，其他内容沿用克制滚动显影；不增加持续循环、滚动劫持或依赖 hover 的信息。

### 关于页 V2：个人自述档案

- **页面任务**：让访客理解旺角西多士如何看待自己的成长、当前学习和希望留下的印象，而不是重复首页简介。
- **结构顺序**：个人主张与事实索引 → 头像和完整成长叙事 → 三段自我理解 → 公开联系方式。
- **真实证据**：22 岁、海南文昌、正在学习成为 AI 训练师、暂用的柴犬头像，以及已确认的成长自述；不补写未经确认的经历节点。
- **视觉焦点**：首屏用一句克制的自我主张领导，右侧以细线档案索引支撑；人物图在第二段独立出现，不与首页同构。
- **内容节奏**：三段自我理解使用有宽窄变化的编辑式行文，不使用等宽卡片、编号或标签眉题。
- **排版规则**：长文保持 45–75ch 阅读宽度；普通段落不低于 16px，章节标题明显低于首页 Hero。
- **响应式规则**：移动端严格单列，人物事实在首屏正文之后、头像之前；不保留桌面端的错位和超大空白。

### 收藏馆 V2：双轨收藏台

- **页面任务**：让访客直接看见当前真实公开的游戏与音乐，并能清楚进入对应内页；不把收藏馆做成四类内容的空导航。
- **结构顺序**：个人化标题与真实统计 → 游戏横幅接触表 → 音乐方形封面接触表 → 最近更新档案 → 书籍与电影的未来说明。
- **首屏证据**：10 条公开记录、4 款游戏、6 首音乐和可计算的 Steam 累计时长；首张真实游戏画面在桌面端与 390px 移动端的第一视口下缘有意露出，提示下方进入真实收藏。
- **媒体语法**：游戏统一使用 16:9 横向画面并采用不对称宽度，音乐保持方形封面与较紧凑节奏；两类内容不再共用一张通用卡片模板。
- **信息规则**：统计只从本地真实数据计算；游戏显示已核实时长，音乐显示歌曲与音乐人；没有本人确认的状态、评分和日期不补写。
- **交互规则**：分类入口、媒体和最近记录都有明确键盘焦点与按压反馈；图片加载失败时仍保留文字、数量与进入内页的路径。
- **空内容规则**：书籍与电影不出现可点击入口，只用自然文案说明“真正留下第一条记录后再出现”。
- **响应式规则**：移动端严格按标题、统计、游戏、音乐、最近记录的顺序单列；游戏与音乐内部媒体网格可以在不溢出的前提下使用两列。

### 游戏页 V2：四段存档

- **页面任务**：Experience 模式下让真实投入时间成为主角，不把游戏页做成推荐榜或评分表。
- **首屏构图**：左侧用累计 2,434.2 小时建立主标题，右侧直接呈现 `Counter-Strike 2` 的 Steam 官方横幅与 1,764 小时核心记录。
- **记录节奏**：其余三款游戏按公开时长依次展开为交错的 16:9 横向记录，用媒体宽窄与信息栏换位形成章节感。
- **反转收尾**：两条本人真实 Steam 评测集中进入一次深色反转，不分散到每条记录，也不补写新评价。
- **信息边界**：只显示已核实的名称、平台、时长与公开评测；未确认的状态、评分、年份和偏好一律省略。
- **响应式与主题**：移动端严格单列，并让第一视口仍包含 CS2 核心记录；暗黑模式沿用既有反转逻辑，保持内容与层级等价。
- **复用边界**：可复用“四段记录、交错横幅、集中引语”的页面节奏，但它仍是游戏页级规则，不提升为全站默认组件；继续使用现有 CSS 与原生显影能力，不新增依赖。

### 音乐页 V2：年度歌单折页

- **页面任务**：Experience 模式下把 2025 年度歌单的六首真实记录一次摊开，让访客先感知封面集合，再逐条浏览歌曲；不把音乐页做成播放器、榜单或推荐系统。
- **批准方向**：以用户批准的 `.impeccable/mocks/music-comp-2.png`“平面六格风琴折”为实现依据，延续 Quiet Reveal 的安静、编辑式气质。
- **首屏构图**：桌面端采用 4/8 非对称布局，左侧窄列承载标题与事实说明，右侧用六张方形封面组成平面折页；细分隔线和 01–06 编号建立连续展开的阅读顺序。
- **记录台账**：首屏之后以 01–06 曲目台账对应六格折页，清楚呈现真实歌曲名称、音乐人、封面和网易云单曲链接。
- **信息与素材边界**：只使用现有六张真实网易云封面和真实歌曲链接；不嵌入或托管音频，不编造播放量、排名、流派、感受或其他偏好数据。
- **响应式与主题**：移动端将折页收敛为 3×2，曲目台账保持清晰单列；暗黑模式与桌面端保持内容和层级等价，并沿用既有 token 与图片失效回退。
- **动效与依赖**：折页和曲目列表复用现有 `Reveal` 能力，遵守减少动态设置；不新增动画库或其他依赖。
